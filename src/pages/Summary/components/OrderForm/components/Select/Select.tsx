...
import { fetchDistricts } from "@/services/api.service";
...
export const Select = ({ setDataForm }: Prop) => {
    const { data } = useFetch(() => fetchDistricts());
    ...
    return (
        <div className="cart-page__select">
            <div
                className="cart-page__input cart-page__selectLabel"
            ...
            <ul
                className={
                    isSelectOpen
                        ? "cart-page__dropdown"
                        : "cart-page__dropdown cart-page__dropdown--close"
                }
                ref={useDropDown}
            >
                {data?.map((districts) => {
