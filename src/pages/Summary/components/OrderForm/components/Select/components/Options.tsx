...
export const Options = ({
    ...
    function selectDistrict() {
        if (refLabel.current && refOption.current && useDropDown.current) {
            refLabel.current.textContent =
                refOption.current?.dataset.value || "";
            useDropDown.current?.classList.add("cart-page__dropdown--close");
            setDataForm((prev) => ({
                ...prev,
                district: refOption.current?.dataset.value || "",
            }));
        }
    }

    return (
        <li onClick={selectDistrict} ref={refOption} data-value={districts}>
            {districts}
        </li>
    );
};
