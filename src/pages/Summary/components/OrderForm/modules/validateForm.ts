...
export const validateForm = ({ dataForm, setErrors }: Props): boolean => {
    ...
    const regexPhoneNumberMinLength = new RegExp(
        RegexPatterns.isPhoneNumberMinLength
    );
    ...
    const lastNameError =
        isRequired(dataForm.lastName) || isAlphabetic(dataForm.lastName);
    ...
    const phoneError =
        isRequired(dataForm.phone) || isValidPhone(dataForm.phone);
